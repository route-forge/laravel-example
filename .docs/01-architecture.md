...
router.push('/catalogs/company-2026');
```
...
| `@route-forge/vue`    | TypeScript | 把 core 包装成 Vue 插件与组合式 API（`useForgeRoute` / `useForgeApi` / `ForgeLink`） | ^2.2       |

> 💡 如果用 React，对应有 `@route-forge/react`；不依赖框架的 SPA 直接用 `@route-forge/core`。
...
Vite 加载 resources/js/app.js
    │
    ▼
app.use(forge) → forge.ready()
    │  ready() 做了什么：
    │  1. 读取 __ROUTE_FORGE__ 摘要
    │  2. 构建 public 层级路由表
    │  3. 注册 provide/inject 与全局属性
    ▼
app.mount('#app') → vue-router 接管 URL
    │
    │  /catalogs/company-2026 → CatalogPage.vue
    │  页面内：
    │  const { route } = useForgeRoute()
    │  const { call } = useForgeApi('public')
    │  route('api.catalogs.show', { slug })   → "/api/catalogs/company-2026"
    │  call('api.catalogs.show', { params })  → JSON 数据
    ▼
...
- **eager vs lazy**：`public` 的路由元信息随 `@forgeSummary` 进首屏 HTML（前台首屏就要用）；
  `manage` 只在登录后由 `useForgeApi('manage')` 按需拉取，不占公开页面体积
...
| **createRouteForgePlugin()**   | `@route-forge/vue` 导出的插件工厂                                                 |
