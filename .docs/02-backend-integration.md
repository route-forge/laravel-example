...
**必须在前端 `app.js` 加载之前**。本项目放在 `</body>` 之前（Blade 中 `@forgeSummary` 之后紧接 `@vite`
的脚本）：
...
<body>
  ...
  @vite(['resources/css/app.css', 'resources/js/app.js'])  {{-- 然后才加载 JS --}}
</body>
```

> ⚠️ 如果你把 `@forgeSummary` 放在 `<head>` 里或 JS 加载之后，前端 `forge.ready()` 会因为读不到
> `window.__FORGE__` 而 reject。
...
前端对应要在 `createRouteForgePlugin`
里传同样的名字（见 [03 · 前端接入](03-frontend-integration.md)）。
...
之后在 Vue 组件里调用 `route()` 时，TypeScript 就会根据路由名自动提示需要哪些参数：
...
会。 **不要把带后台权限控制的路由名暴露在 forge 上下文里**。用 `exclude_patterns`
排除管理后台路由，或者只在有对应权限的页面里渲染 `@forgeSummary`（可以用 Blade 的 `@can` 包裹）。
