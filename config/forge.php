...
return [
    ...
    */
    'levels'            => [
        /*
        | 前台：唯一 Blade 壳页 + 公开 JSON 接口，浏览器首屏就要用，故 load = eager。
        |
        | 归级全部走显式 tier（SPEC §3.1.4 优先级最高）：
        |   ① routes/web.php 的壳页路由用 group tier 声明为 public；
        |   ② routes/api.php 的公开接口组用 $r->tier('public') 声明；
        |   ③ 登录接口 POST /api/auth/login 单独 ->tier('public')（登录时无会话，必须放行）。
        |
        | ⚠ match.prefix 刻意留空：若配 ['api']，未显式归级的 manage 路由会被 public
        |   抢先吞掉 —— strict_mode 就失去了「漏归级直接 500」的兜底意义。
        */
        'public' => [
            'description' => '企业画册前台页面与公开数据接口（无需登录）',
            'match' => [
                'prefix'     => [],
                'middleware' => [],
            ],
        ...
        /*
        | 管理端：企业维护画册/页面/留言的接口（URI 统一在 /api/manage 下），load = lazy ——
        | 明细不进公开 HTML，前端登录后再按层级懒加载（「懒加载 + 受保护」的用法本身）。
        |
        | 归级双保险：routes/api.php 的 manage 组显式 tier('manage') + 本处
        | match.prefix = ['api/manage']（将来新路由漏写 tier 也归不到 public）。
        |
        | 鉴权两层：
        |   - 请求拦截：manage 组挂 web（会话）+ manage（EnsureManageAccess，登录 + is_manager）；
        |   - 元信息保护：endpoint_middleware 让未登录者连 GET /_forge/routes/manage
        |     都拿不到 —— 「后台有哪些路由」本身就是信息。
        | 注意 prefix 与 middleware 是 OR 关系，两条任一命中即归入本层级。
        */
        'manage' => [
            'description' => '企业管理端接口（画册、页面、留言，需登录）',
            'match' => [
                'prefix'     => ['api/manage'],
                'middleware' => ['manage'],
            ],
    ...
    */
    ...
    |
    | 本示例取 true（DESIGN.md §7 推荐生产开启）：漏写 tier 时宁可直接 500 报错，
    | 也不要静默掉进 unassigned —— unassigned 路由不进 route:forge:types，
    | 前端会「路由明明在、类型和调用都不通」，排查成本远高于一次异常。
    |
    */
    'strict_mode'       => env('FORGE_STRICT_MODE', true),
