...
{{--
  ...
  这里只负责三件事 —— 注入 forge 摘要、注入 CSRF token、挂载 React。
  页面路由（/catalogs/xxx、/manage/messages …）全部由前端 react-router 维护，
...
<head>
  ...
  <meta name="description" content="route-forge × Laravel × React 19 企业画册示例">
  ...
  {{-- @forgeSummary 放进 <head>、早于 @vite：它输出的是一段一次性 window 访问器脚本
       ...
       app.jsx 是 type=module（defer），放在这里才是契约本意，也避免以后有人改成
       非 defer 脚本时静默失效。 --}}
  @forgeSummary
  @fonts
...
{{-- #app 内的占位块会在 React 挂载时被整体替换，只为避免首屏纯白 --}}
<div id="app">
  <style>
    .boot-loading {
      display: flex;
...
<noscript>
  <p class="p-6 text-center text-sm text-gray-500">
    本站内容全部由 React 渲染，需要开启 JavaScript 后查看。
  </p>
</noscript>
</body>
</html>
